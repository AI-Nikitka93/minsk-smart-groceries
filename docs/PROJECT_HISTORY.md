...
Дата и время: 2026-03-30 18:29
Роль: P-BOT Universal Bot Architect
Сделано: Построен и запущен первый evaluator-backed golden-set harness для `bot-worker`: добавлены live cases, TypeScript runner, package script, baseline document и machine-readable result artifact; параллельно исправлен кириллический regex bug в profile-only router, из-за которого `у меня диабет и бюджет 30 рублей нас 4` не всегда распознавалось как `save_profile`.
Изменены файлы: AGENTS.md, package.json, src/apps/bot-worker/index.ts, src/apps/bot-worker/golden-set.cases.json, src/apps/bot-worker/golden-set.ts, docs/GOLDEN_SET_BASELINE_2026-03-30.md, docs/evals/golden-set-latest.json, docs/STATE.md, docs/state.json, docs/EXEC_PLAN.md, docs/DECISIONS.md, docs/PROJECT_HISTORY.md
Результат/доказательство: `npm run typecheck` -> success; `npm run eval:golden` -> pass rate `3/3`; live result artifact `docs/evals/golden-set-latest.json`; worker deploy `1e848823-d64a-4280-9cb0-d7a231bf673d`; baseline cases passed: `profile_save_household`, `cheapest_followup_milk`, `basket_week_diabetes`
Следующий шаг: Расширить golden-set до v2: staple exactness, basket realism, diagnosis-aware follow-ups, clarification quality и safer-swap regressions.
